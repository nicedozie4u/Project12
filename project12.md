...
1. We have our nice and clean `dev` environment, so let us put it aside and configure 2 new Web Servers as `uat`. We could write tasks to configure Web Servers in the same playbook, but it would be too messy, instead, we will use a dedicated `role` to make our configuration reusable.

Launch 2 fresh EC2 instances using RHEL 8 image, we will use them as our `uat` servers, so give them names accordingly – `Web1-UAT` and `Web2-UAT`.

![](./images/uat%20instances.PNG)
...
![](./images/edit%20role%20files.PNG)
...
![](./images/update%20uat%20files.PNG)

4. In `/etc/ansible/ansible.cfg` file uncomment `roles_path` string and provide a full path to your roles directory `roles_path    = /home/ubuntu/ansible-config-mgt/roles`, so Ansible could know where to find configured roles.

![](./images/edit%20ansible%2Ccfg.PNG)

![](./images/edit%20ansible%2Ccfg02.PNG)
...
![](./images/create%20uat%20assignment.PNG)
...
![](./images/final%20playbook%20success.PNG)

![](./images/jenkins%20push%20update.PNG)

![](./images/jenkins%20push%20update02.PNG)
